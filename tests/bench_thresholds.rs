use cielo::analysis::bench_thresholds::{
    check_v1_pipeline_per_iter_ms, v1_pipeline_per_iter_ms_limit, v1_pipeline_thresholds,
};
...
fn v1_baseline_measurements_fit_thresholds() {
    ...
    for (case, per_iter_ms) in baseline_per_iter_ms {
        assert!(
            check_v1_pipeline_per_iter_ms(case, per_iter_ms).is_ok(),
...
fn threshold_checker_rejects_regressions() {
    let violation = check_v1_pipeline_per_iter_ms("direct_resume", 1.000)
...
fn thresholds_slice_is_stable_and_non_empty() {
    let thresholds = v1_pipeline_thresholds();
    assert!(
        !thresholds.is_empty(),
